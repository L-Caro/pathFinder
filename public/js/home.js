...
// = Réinitialiser la recherche de texte =
    const searchText = document.querySelector('.button');                      // Récupérer l'élément de recherche de texte
...
// =====================================================================
// =====================================================================
// == Fonction recherche par nom version desktop == //
function filterTreksByName() {                                                        // Créer une fonction pour filtrer les treks par nom
  const searchInput = document.querySelector('.search-desk-button');                       // Récupérer l'input de recherche
...
const submitButton = document.querySelector('.search-desk-submit');                        // Récupérer le bouton de recherche
...
const inputButton = document.querySelector('.search-desk-button');                        // Récupérer le bouton de recherche
...
//! =====================================================================
// == Fonction recherche par nom version mobile == //
function filterTreksMobByName() {                                                        // Créer une fonction pour filtrer les treks par nom
  const searchInput = document.querySelector('.search-mob-button');                       // Récupérer l'input de recherche
  const searchText = searchInput.value.toLowerCase();                                 // Récupérer la valeur de l'input de recherche et la mettre en minuscule
  const allTreks = document.querySelectorAll('.trek-home');                           // Récupérer tous les treks dans une variable
  const result = [];                                                                  // Créer un tableau vide qui contiendra les treks qui correspondent à la recherche
...
const submitMobButton = document.querySelector('.search-mob-submit');                        // Récupérer le bouton de recherche
submitMobButton.addEventListener('click', () => {                                        // Ajouter un écouteur d'événement sur le bouton de recherche
  filterTreksMobByName();                                                                // Quand on clique sur le bouton de recherche, on appelle la fonction filterTreksByName
  updatePosition();                                                                   // Et la fonction updatePosition pour que le footer s'adapte
});
const inputMobButton = document.querySelector('.search-mob-button');                        // Récupérer le bouton de recherche
inputMobButton.addEventListener("keydown", function(event) {
  if (event.keyCode === 13) { // 13 correspond à la touche "Enter"
    filterTreksMobByName();                                                                // Quand on clique sur le bouton de recherche, on appelle la fonction filterTreksByName
    updatePosition();                                                                   // Et la fonction updatePosition pour que le footer s'adapte  
  }
